...
inf_gr <- inf_df |>
  group_by(.class_gr, .equivocal_gr) |>
  summarise(count = n(), file_list = list(file_name)) |>
  ungroup() |>
  mutate(prop = count / sum(count))

inf_cl <- inf_df |>
  group_by(.class_cl, .equivocal_cl) |>
  summarise(count = n(), file_list = list(file_name)) |>
  ungroup() |>
  mutate(prop = count / sum(count))

inf_bb <- inf_df |>
  group_by(.class_bb, .equivocal_bb) |>
  summarise(count = n(), file_list = list(file_name)) |>
  ungroup() |>
  mutate(prop = count / sum(count))
...
inf_no <- inf_df |>
  group_by(.class_no, .equivocal_no) |>
  summarise(count = n(), file_list = list(file_name)) |>
  ungroup() |>
  mutate(prop = count / sum(count))

inf_gr_files <- inf_gr |>
  filter(.equivocal_gr == "Unequivocal") |>
  rowwise() |>
  mutate(file_samp = list(sample(file_list, 500)),
         type = "gr",
         class = .class_gr)

inf_cl_files <- inf_cl |>
  filter(.equivocal_cl == "Unequivocal") |>
  rowwise() |>
  mutate(file_samp = list(sample(file_list, 500)),
         type = "cl",
         class = .class_cl)
...
inf_bb_files <- inf_bb |>
  filter(.equivocal_bb == "Unequivocal") |>
  rowwise() |>
  mutate(file_samp = list(sample(file_list, 500)),
         type = "bb",
         class = .class_bb)
...
inf_no_files <- inf_no |>
  filter(.equivocal_no == "Unequivocal") |>
  rowwise() |>
  mutate(file_samp = list(sample(file_list, 500)),
         type = "no",
         class = .class_no)
