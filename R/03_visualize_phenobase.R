...
p2_fruit = p2_fruit + scale_fill_manual(values = c("darkred", "lightblue")) +
  scale_color_manual(values = c("darkred", "lightblue"))
p2_fruit
...
ggplot() + 
  geom_sf(data = wld, fill = "gray", color = "gray") +
  geom_sf(data = grids2, inherit.aes = F)
...
dat = st_transform(st_as_sf(coverage_inat, coords = c("longitude", "latitude"), crs = 4326, remove = F), 
                   crs = "ESRI:54009")
if(!file.exist("data/dat_inat_coverage_grids.rds")){
  dat1 = st_join(dat, grids)
  saveRDS(dat1, "data/dat_inat_coverage_grids.rds")
} else {
  dat1 = readRDS("data/dat_inat_coverage_grids.rds")
}
