...
ARGS=("--N" "900" "--L" "14" "--Lm" "8" "--burn-in" "600" "--tau" "100" "--replicates" "192" "--regime" "exploration")
if [[ $# -gt 0 ]]; then ARGS=("$@"); fi

echo "[v3] args: ${ARGS[*]}"
mamba run -n two_noise_jax python -m sim.v5_adiabatic "${ARGS[@]}" \
    2>&1 | tee logs/v5_adiabatic_$(date +%Y%m%d_%H%M%S).log

echo "[v3] results:"; cat runs/v5_adiabatic/results.csv || true
