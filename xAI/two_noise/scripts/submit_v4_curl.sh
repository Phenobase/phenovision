...
ARGS=("--N" "700" "--L" "12" "--burn-in" "450" "--tau" "80" "--replicates" "160" "--grid" "0.03" "0.05" "0.07" "0.09")
if [[ $# -gt 0 ]]; then ARGS=("$@"); fi

echo "[v3] args: ${ARGS[*]}"
mamba run -n two_noise_jax python -m sim.v4_curl "${ARGS[@]}" \
    2>&1 | tee logs/v4_curl_$(date +%Y%m%d_%H%M%S).log

echo "[v3] results:"; cat runs/v4_curl/results.csv || true
