#!/bin/bash
# V3 — production-scale M-drift direction test (the three M-evolution regimes (canalization / emergent-bet-hedging exploration / tracking / intermediate).
...
ARGS=("--N" "1200" "--L" "16" "--burn-in" "700" "--tau" "120" "--replicates" "384" "--anisotropy" "4.0" "--chal-strength" "1.0" "--chal-sigma" "1.5")
if [[ $# -gt 0 ]]; then ARGS=("$@"); fi

echo "[v3] args: ${ARGS[*]}"
mamba run -n two_noise_jax python -m sim.three_regimes "${ARGS[@]}" \
    2>&1 | tee logs/three_regimes_$(date +%Y%m%d_%H%M%S).log

echo "[v3] results:"; cat runs/three_regimes/results.csv || true
